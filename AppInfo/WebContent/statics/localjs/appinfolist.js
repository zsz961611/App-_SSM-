$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/AppInfo/devapp/one.json",//请求的url
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/AppInfo/devapp/two.json",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/AppInfo/devapp/appversionadd.html?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="../devapp/appversionmodify.html?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="../devapp/appinfomodify.html?id="+ obj.attr("appinfoid");
	}else{
...
$(document).on("click",".saleSwichOpen,.saleSwichClose",function(){
	...
	if("open" === saleSwitch){
		...
		saleSwitchAjax(appinfoid,status,saleSwitch);
...
var saleSwitchAjax = function(appId,status,saleSwitch){
	
	$.ajax({
		type:"PUT",
		url:"../devapp/updateStatus.json?appId="+appId+"&status="+status,
		dataType:"json",
...
/*$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="/devapp/lookapp.html/"+ obj.attr("appinfoid");
});*/
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:"/AppInfo/devapp/delapp.json",
